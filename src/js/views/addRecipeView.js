...
class AddRecipeView extends View {
  ...
  _addHandlerHideWindow() {
    ...
    window.addEventListener('keydown', this._closeWindowEsc.bind(this));
  }
      ...
      ingredients.forEach((ing, i) => {
        if (ing.length !== 3)
          return this.insertAdjacentHTML(
            'beforeend',
            `Ingredient ${i + 1} was formated wrong! Please fix it`
          );
      });
  ...
  _generateMarkup() {
    return `
      <div class="upload__column">
        ...
        <input value="TEST23" required="" name="img" type="text">
      ...
      <button class="btn upload__btn">
        <svg>
          <use href="/icons.b3083592.svg#icon-upload-cloud"></use>
        </svg>
        <span>Upload</span>
      </button>
    `;
